...
- `SESSION_MAX_AGE_DAYS`（預設 30）：限制外洩 DB 的可用時效，行為類似**閒置逾時**。
  ...
  載入時** lazy 觸發，沒有常駐計時器。
- `SESSION_DEATH_OBSERVE_MINUTES`（預設 90，`0` 關閉）：保活連續失敗 5 分鐘後，
  **先通知使用者但不丟掉 session**，繼續探測這麼久，看那個「未登入」的判定會不會
  自己恢復。使用者收到通知的時機完全沒變，只有「真的丟掉」往後延。理由見
  〈保活判死：一個還沒被驗證的假設〉。預設 90 分鐘是為了蓋過**已公告的一小時維護**
  加上發現延遲與逾時，不是那個 5 分鐘的 IP 罰則。觀察期間 `/status` 會誠實顯示
  「保活異常觀察中」，不會謊報已登入。
...
npm run probe:rate -- --go    # 找風控天花板：在哪被拒、拒絕長什麼樣、範圍多寬、多久解除
npm run probe:rate -- --go --only=key --gap=250 --max=8            # 配額（灌滿，第 Q+1 發被拒）
npm run probe:rate -- --go --only=key --gap=60000 --decay=2000 --step=1 --max=40  # 窗寬（階梯）
npm run probe:isolation       # 同時開兩場登入，確認核准落在對的那一場
```
...
**`check:ggm` 會直接問伺服器**：我們送的 `CV`/`Hash` 還被接受嗎？同時比對上游
`pungin/Beanfun` 維護的 `ggm-client.json` 與 Beanfun 自己的 `CheckVersion.ashx`。
量測壓過比對 —— 版本差異只是對「還接不接受」的猜測，canary 是直接問。

canary 就是一個**帶著隨機廢票**的 POST。該端點在檢查票之前就先檢查啟動器身分，而且
對匿名呼叫者也一樣，所以 `Client_Integrity_Failed` 與 `Invalid_Start_Ticket` 的差別
就能回答「我們這組死了沒」——不需要 session、不需要使用者、不產生任何 OTP。

**所以這個檢查有排程（每日 + 啟動時），而版本比對從來沒有**：它量的是故障本身而不是
領先指標，因此在真的壞掉之前不會出聲。設定 `OWNER_DISCORD_ID` 就會在壞掉時收到一次
DM（**一次故障只講一次**，不是每天）。沒有它就沒人會發現：使用者選的遊戲會被持久化
並跨重啟保留，所以一個大家都停在 legacy 遊戲的部署可能好幾個月不發任何 v2 請求，
第一個訊號會是一個說不清楚發生什麼事的使用者。

## 風控：登入端點本身也有配額

Beanfun 對 `bflogin/default.aspx` 有 **per-IP** 的配額，而一個部署把所有使用者放在
同一個對外位址 —— 所以一個人手快，全部人一起被拒。從一條可以重撥的線路上故意撞出來的
結果：

- **整條鏈上只有這一個 endpoint 有閘。** 保活 180 發（一次 60 並發）、列帳號 60 發
  （一次 20 並發）、輪詢 80 發連打，全部沒事。被擋時這些也照常回應，所以事故範圍是
  「無法開始新登入」，已登入的人不受影響。
- **拒絕是 HTTP 200**，導向 `/TW/BlockIPMessage.htm`；檔名只在 URL，
  `IP已自動被系統鎖定` 只在 body，兩個都檢查。
- **數次數不數速率**，罰則 4.5～4.8 分鐘且不會被後續請求延長。
- **配額 4 個 / 80 秒滑動窗。** 配額由背對背灌滿直接量到（第 5 發被拒，不需假設窗寬）；
  窗寬由兩趟形狀不同、相隔兩天的階梯各自夾出區間，**交集只有 2.4 秒寬**。

限流器是**每 30 秒一發**，共用一個 FIFO 佇列。以這個間隔送出時，還留在 beanfun 那個 80 秒窗
裡的只有 −30 秒和 −60 秒兩發 —— 伺服器看到 2 發，而它要 4 發才拒絕。

**它是間隔制而不是配額制，原因是重啟**：我們的計數在記憶體裡，beanfun 的不是。配額制會把
整份額度發給先來的人，所以「3 人登入 → 重新部署 → 再 3 人登入」會把 6 發擠進同一個 80 秒窗
而被拒。間隔制沒有 burst 可以被重啟還回來，開機時再先佔用一格就連漏一發都堵掉。代價是
同時登入的第 2 個人要等 30 秒。

**擋不住的是鄰居**：線上走共用出口，別人的 pSKey 落進同一個 per-IP 計數器，我們看不到。
所以限流器把「被拒絕」當成唯一可信的校正：撞到就自己關禁閉五分鐘，並把當下我們自己的
足跡印進 log —— 那是唯一能分辨「鄰居花掉的」和「這裡配額本來就更小」的東西。

完整的量測方法與資料在
[`docs/PROTOCOL-FLOW.md`](./docs/PROTOCOL-FLOW.md)〈階段 0 的閘〉。
