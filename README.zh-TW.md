...
npm run probe:rate -- --go --arm=window --search   # 從下面探計數窗口的寬度
...
同一個對外位址 —— 所以一個人手快，全部人一起被拒。2026-08-19 從一條可以重撥的線路上
故意撞出來的結果：

- **整條鏈上只有這一個 endpoint 有閘。** 保活 180 發（一次 60 並發）、列帳號 60 發
  （一次 20 並發）、輪詢 80 發連打，全部沒事。
- **拒絕是 HTTP 200**，導向 `/TW/BlockIPMessage.htm`；檔名只在 URL，
  `IP已自動被系統鎖定` 只在 body，兩個都檢查。
- **數次數不數速率**，罰則固定 4～5 分鐘且不會被後續請求延長。

**配額與窗口寬度刻意不寫進來，因為它們會動**：同一個 IP，早上量到「80 秒內 4 個」，
下午卻連「第一個之後 177 秒」都還在拒 —— 沒有任何一組固定參數能同時解釋兩次。風控
看起來會針對位址的近期行為收緊，這也足以解釋線上部署為什麼會撞到，不需要假設機房 IP
額度比較低。

所以限流器把「被拒絕」當成唯一可信的校正：撞到就自己關禁閉五分鐘，並把當下我們自己
的足跡印進 log —— 那是唯一能分辨「額度被鄰居用掉」和「這裡配額本來就更小」的東西。
在別的地方、別的時間量到的常數，不是關於現在的證據。
