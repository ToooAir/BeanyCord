...
- **罰則約 4.5～4.8 分鐘，而且不會被後續請求延長。** 每次量到的是一個區間（每 30 秒
  重探一次，所以粒度是 0.5 分鐘）。2026-08-21 兩趟分別得到 **[4.5, 5.4]** 與
  **[4.0, 4.8]**，**相交於 [4.5, 4.8] 分鐘**；2026-08-19 量到的 4.0～4.9 也涵蓋它。
  罰則期間繼續打不會讓它變久，所以探測恢復是免費的。`IP_BLOCK_PENALTY_MS = 5 分鐘`
  涵蓋這個上界，不需要再加大。
- **只擋登入入口。** 每次撞牆後都用全新 client 掃過一輪，三次結果一致：`echo_token.ashx`、
  `game_start_step2.aspx`、`game_zone/` 與兩個 host 的大門**全部照常回應**，只有
  `default.aspx` 被擋。事故範圍是「無法開始新登入」，已登入的人不受影響。

### 配額與窗口寬度：**4 個 / 80 秒滑動窗**

2026-08-21 用兩個互補的實驗量到，各自量對方量不到的那一半：

| 何時 | 方法 | 結果 |
| --- | --- | --- |
| 2026-08-19 11:36Z | 階梯（幾何，×0.75），27 次成功後被拒 | W ∈ [79.2, 83.6]，**前提是 Q=4** |
| 2026-08-21 09:45Z | 背對背灌滿（`--gap=250`），第 5 發被拒 | **Q = 4**，不需假設窗寬 |
| 2026-08-21 10:19Z | 階梯（線性，−2s/發），24 次成功後被拒 | W ∈ [71.5, 81.6]，Q=4 |

兩次階梯**形狀完全不同、相隔兩天**，區間卻**相交於 [79.2, 81.6]（僅 2.4 秒寬）**。
兩個獨立實驗互相夾出同一個值，比任何單次測量都可信。

**沒有第二層長時距計數器。** 2026-08-21 那趟在任意 120 秒內被服務 6 發、300 秒內 11 發、
900 秒內 23 發，全部放行 —— 唯一在管的就是那個 80 秒窗。

#### 被推翻的舊結論

先前這一節寫的是「配額與窗口尚未定案，而且會動」，依據是 2026-08-19 15:00Z 用
`--arm=window --search` 得到的「W ≥ 177 秒」。**那個結果現在被推翻**：它從頭到尾沒有任何
一次試驗被放行，所以上界從來沒被證實過，二分搜尋只是收斂到當初傳進去的 `--hi` 上。腳本
現在會明說「未建立上界」而不是印出一個賺不到的區間。

連帶地，「風控會針對近期行為收緊窗口」這個假說**沒有得到支持**：2026-08-21 那趟是在同一條
線**被封鎖後約 34 分鐘**開始跑的，量到的 W 精準落回 8/19 的值。

### 限流器：每 30 秒一發，排隊等候

`sessionKey.ts` 用 `SlidingWindow(1, 30s)` —— 上限一發就是最小間隔，不需要另一套機制 ——
再套上共用的 FIFO `RateGate`。

**為什麼 30 秒安全**：以嚴格 30 秒間隔送出時，還留在 beanfun 那個 80 秒窗裡的只有 −30 秒和
−60 秒那兩發（−90 秒那發已經滑出）。所以**伺服器看到 2 發，而它要 4 發才拒絕**。這和先前
3/120s 的餘裕完全相同，但吞吐量多了三分之一。區間兩端（79.2 秒、83.6 秒）都成立；窗要再
變寬 50% 才會塞得下 4 發，若配額其實是 3 則是 12%。

**為什麼是間隔而不是配額**：因為重啟。我們的計數在記憶體裡，beanfun 的不是。配額制會把
整份額度發給先來的人，所以「3 個人登入 → 重新部署 → 再 3 個人登入」會把 **6 發**擠進同一個
80 秒窗，第 5 發被拒 —— 對著實測參數模擬，幾乎在任何重啟時點都會發生。間隔制沒有 burst
可以被重啟還回來，最多漏一發，而開機時先佔用一格（`qrWindow.take()`）連那一發也堵掉，
代價是每次部署後的第一次登入等 30 秒。

**代價**：同時登入的第 2 個人要等 30 秒、第 3 個等 60 秒，而不是三個人同時拿到。對一個使用者
會連續登入好幾天的 bot 來說登入本來就稀疏，用它換掉全體五分鐘的停擺是划算的。

**擋不住的東西**：鄰居。線上走共用出口，別人的 pSKey 落進同一個 per-IP 計數器，任何我們自己
的預算都看不到。所以被拒絕才是唯一可信的校正訊號（`RateGate.penalise()`），並把當下我們自己
的足跡一起印進 log —— 那是唯一能分辨「鄰居花掉的」和「這裡配額本來就更小」的東西。

### 線上那次被拒：證據太弱，不列入計算

先前這裡把 2026-08-19 線上被拒當成「那個位址參數不同」的證據，推論出配額可能是 3 或窗口
≥90 秒。**現在撤回。**

那筆觀測裡的四個請求年齡（55 / 67 / 79 / 90 秒）**不是從 log 讀出來的**，是根據使用者回憶
「按了幾次」反推的 —— `footprint()` 當時還沒部署，所以那個位址當下真正的請求計數從來沒有被
記錄過。而那天 production 正在被反覆測試與重新部署，有多少發沒被算進那份重建清單無從得知。

用一筆未經儀器記錄、且發生在當天被猛打過的位址上的重建資料，去推翻兩次乾淨的家用線量測，
推論方向是反的。剩下唯一站得住的解釋是共用出口的鄰居 —— 而那本來就不是靜態預算能防的。

要真正解決，就是**在線上那個位址上直接量**：`--gap=250 --max=8` 的灌滿只要 5 個請求、約
15 秒，代價是新登入停 5 分鐘（已登入的人不受影響，量過三次）。挑離峰時段跑一次就有答案。

### 方法論：一個實驗只能量到一半

**「被擋後多久恢復」量不到窗口**，那量到的是**罰則** —— 任何觸發拒絕的實驗都會立刻進入
4～5 分鐘禁閉，訊號被蓋掉。

**背對背灌滿量配額，量不到窗口。** 所有請求都擠在十幾秒內，對「30 秒到 900 秒」的每一個
候選窗來說計數都一樣，完全沒有鑑別力。它的產出就是「第 Q+1 發被拒 ⟹ 配額 = Q」，不需要
假設任何窗寬。腳本現在會偵測這種 run（整趟跨距 ≤ 最窄候選窗）並拒絕印出賺不到的區間。

**階梯量窗口，量不到配額。** 從一個安全的間隔開始逐步收緊，撞牆時兩邊夾出 W：

- **下界** —— 被拒的那發證明前面 Q 發全在窗內，所以 `W ≥ t_拒 − t_(n−Q)`
- **上界** —— 每一發被放行的都證明它前面 Q 發沒能全塞進窗裡，取最緊的那組

但它**分不出配額**：真實的 (W=80, Q=4) 在 Q=3 的假設下會讀成同樣自洽的 (W=57, Q=3)。這是
結構性的 —— 階梯只看得到分散的請求，所以它量的是配額**補充多快**，永遠不是它**多深**。
腳本會把兩種讀法都印出來並明說無法選擇；跨讀法能穩住的是速率 Q/W（差 2～5%）。

**遞減量決定精度，起跑間隔決定量程。** 區間寬度 = `Q × 每發遞減量`（−2s → 8 秒寬）。而起跑
間隔決定「最寬能量到多寬的窗」（約 `4 × 起跑間隔`）—— 從接近已知值的地方起跑，等於把待驗證
的數字寫進實驗設計裡：窗若比預期寬，第 5 發就撞牆、連上界都算不出來，白燒一次 IP。

重現這兩趟：

```bash
npm run probe:rate -- --go --only=key --gap=250 --max=8 --write                       # 配額
npm run probe:rate -- --go --only=key --gap=60000 --decay=2000 --step=1 --max=40 --write  # 窗寬
```

兩趟之間要等罰則退掉再加一整個窗（保守抓 10 分鐘）。**只在可以重撥換 IP 的線路上跑**。
...
登入功能維護**，而兩個使用者是在 **08:06 TW**（= UTC+2 的 02:06）同一分鐘被判登出 ——
維護開始後 6 分鐘。不是風控、不是我們的 bug。

同一輪檢查中被排除的其他解釋，留著備查：

| 假說 | 為什麼排除 |
| --- | --- |
| ping 被風控擋掉 | 被擋是 HTML／非 2xx，`isLoggedOutEcho` 認不出來就當活著，**產生不出 `session.logged_out` 這行 log**。而且實測「IP 封鎖只擋登入入口，已登入的人不受影響」（見〈階段 0 的閘〉） |
| 「5 分鐘」與封鎖罰則同步 | 那 5 分鐘是我們自己的 `PING_FAIL_THRESHOLD × PING_INTERVAL_MS`，不是量到的東西 |
| session 有絕對壽命 | 使用者回報同一部署曾放超過兩星期 |
| 有人重複登入把舊 session 踢掉 | 兩位使用者都確認半夜沒有動作 |
| 兩個 client 共用狀態 | 每位使用者各自 `new CookieJar()`，`capture/isolation/` 實測過同 IP 兩場不串線 |

**教訓：集體同時登出，先去查對方的維護公告。** 這一條在推論鏈上比任何測量都便宜，而我們
把它當成「除非他們改規則」給跳過了。三天前那次也應該用同樣的方式核對一次公告。

### 還沒結案的那一種

**單一 session 死亡：A 死、B 沒死**，A 的持有者確認當時沒有人登入、自己也沒有。集體登出的
所有解釋（維護、出口 IP、後端節點）都預測**全體一起死**，所以這是另一回事。它只有兩種可能，
而且觀察窗口正好能分開：

1. **暫時性判定**（短暫故障、被移到不認得這個 session 的節點），只是剛好連續 5 次。
2. **真的只有那一個 session 被殺**（原因不明）。

而我們從來沒有驗證過 `ResultCode:0` 是不是永久的。`capture/dead/` 是我們**刻意殺掉**的
session；暫時性判定在 wire 上完全一樣。舊的迴圈在第 5 分鐘丟掉 session 之後就再也沒回頭
看，所以它連原則上都分不出這兩者。

順帶一提，**「同一部署曾放超過兩星期」這個對照可能無效**：`ping()` 在 `c67bea6`
（2026-08-13）之前根本不讀 body，而 `echo_token` 對死掉的 session 也回 200 —— 保活
**在結構上不可能**觸發那個 drop。兩星期量到的可能是「多久之後才有人去用它」。
...
- 沒有任何一個現存假說預測 ping 的頻率會影響存活。參考實作（WPF `pingWorker`）就是單發
  60 秒；維護、出口 IP、後端節點，打快六倍都救不回來。
- 沒有對照組就讀不出結論。全部一起加速的話，「活下來」不能歸功於加速，「死掉」也分不清
  是被打死的還是本來就會死。真要測只能 A/B —— 兩個 session 不同頻率，若仍同時死，頻率
  就不是變數。
- 拿**真實 session** 去撞 `echo_token` 的上界，就把 `rateProbe.ts` 刻意守住的那條線跨過去
  了：那支探針的設計原則是**只賭 IP，不賭帳號**。而 180 並發那個數字是 8/19 單線一次量到
  的**上界**，不是預算。
...
| `npm run probe:rate -- --go` | **找風控的天花板**：在哪裡被拒、拒絕長什麼樣、封鎖範圍多寬、持續多久。配額與窗寬要用兩趟分開量（`--gap=250` 灌滿量配額、`--decay=` 階梯量窗寬，見〈階段 0 的閘〉）；`--arm=hammer --target=…` 單獨測某個 endpoint 有沒有自己的閘 |
